...
$view = views_get_view_result('next');
//dsm($view);
$next_array = array();
$search_string = '/regular_dolls_list/'; //change here to find a different content type
foreach ($view as $view_item){
	$id = $view_item->node_field_data_field_regular_doll_list_nid;
	$url = url('node/'.$id);
	if (strpos($url, $search_string)!==false){
		//$dolls_list_array[$id]=$url;
		array_push($next_array, $id);
		}
	}
//dsm($dolls_list_array);
$this_nid = $fields['nid']->content;
$key = array_search($this_nid, $next_array);
if ($key == 0)
	$prev_url = url('node/'.$next_array[count($next_array)-1]);
else {
	$prev_index = $key-1;
	$prev_url = url('node/'.$next_array[$prev_index]);
	}
if ($key == count($next_array)-1){
	$next_url = url('node/'.$next_array[0]);
	}
else {
	$next_index = $key+1;
	$next_url = url('node/'.$next_array[$next_index]);
	}
?>

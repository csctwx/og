...
function og_preprocess_html(&$variables){
	
	$variables['keywords'] = 'Doll, dolls, doll clothes,  American Girl, American Girl doll clothes, dolls for girls, 18inch dolls';
	$variables['description'] = 'There has never been a generation of girls quite like us. Our Generation 18inch dolls, doll clothes and accessories.';
	...
	if ($node = menu_get_object()) {
		...
		if ($node && isset($node->nid)) {
			...
			if (!empty($node->field_meta_keywords)) {
				$variables['keywords'] = $node->field_meta_keywords['und'][0]['value']; 
			}
			
			if (!empty($node->field_meta_description)) {
				$variables['description'] = $node->field_meta_description['und'][0]['value']; 
			}
		}
	}
...
function node_sibling($dir = 'next', $node, $next_node_text=NULL, $prepend_text=NULL, $append_text=NULL){
    ...
    $row = db_query($query, array(':created' => $node->created, ':type' => 'word_of_the_day'))->fetchObject();

  if($row) { 
  
  	// If there is a node add prefixes and return it  
    $text = $next_node_text ? $next_node_text : $row->title;
    return $prepend_text . l($text, 'node/'.$row->nid, array('rel' => $dir)) . $append_text;
    
  } else {
  
     // If there isn't a Next or Prev this will return the newest or oldest node in a new query - EIE
     $query = 'SELECT n.nid, n.title FROM {node} n WHERE '
           . 'n.type = :type AND n.status=1 '
           . 'ORDER BY n.created ' . ($dir == 'previous' ? 'DESC' : 'ASC');
     $row = db_query($query, array(':type' => 'word_of_the_day'))->fetchObject();
     $text = $next_node_text ? $next_node_text : $row->title;
     return $prepend_text . l($text, 'node/'.$row->nid, array('rel' => $dir)) . $append_text;
...
function og_preprocess_views_view(&$vars) {
  $view = &$vars['view'];
  // Make sure it's the correct view
  if ($view->name == 'videos') {
    // add needed javascript
    drupal_add_js(drupal_get_path('theme', 'og') . '/js/html5gallery.js');
    // add needed stylesheet
    drupal_add_css(drupal_get_path('theme', 'og') .'/css/video.css');
  }
}
